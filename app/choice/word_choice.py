...
def word_choice(filter):
    ...
    # Группы формируются на основе списка букв отсортироанных по количеству слов содержащих эту букву
    # Для этого формируется набор букв (начиная с начала списка), выводятся слова состоящие из них,
    # далее добавляется буква из списка и все повторяется (выведенные слова повторно не выводятся)
    number_group = dict()
    summ = 0
